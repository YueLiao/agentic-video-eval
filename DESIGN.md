...
这两个要求互相拉扯:**任何拉开分数的改动都会同时放大喂给它的噪声**。
所以「分差变大了」本身不构成任何证据——把同一个比例整体放大,信噪比一点没变。

唯一有意义的判据是三个数一起看:

| | 含义 | 怎么测 |
|---|---|---|
| **组间** | 不同视频之间的分差 = 区分度 | 各视频均分的标准差 |
| **组内** | 同一视频重复跑的分差 = 复现性 | 同一视频多次运行的标准差 |
| **信噪比** | 组间 / 组内 | 只有它变大才算真的改善 |
| **排序稳定** | 重复中模型排序是否一致 | 翻转过的 aspect 撑不起榜单列 |

重复必须**扰动无关变量**,不能只是重跑:temperature=0 且 prompt 固定时贪心解码是
确定的,重跑三次会得到逐位相同的结果,那测的是「确定性解码是否确定」(是),不是稳定性。
实际扰动的是**采样相位**——看哪几帧是个任意选择,不该影响结论。

`scripts/run_stability.py` 同时跑多种配置(单相位 vs 多相位共识),
所以每个降噪机制是否值得它的成本是**被测出来的,不是被假设的**。

### 已知会伤害稳定性的两个改动

记在这里,因为它们都是为了区分度做的,代价必须一并核算:

1. **取最差 k 个 aspect 作为总分**(而非全部平均)。最差的 aspect 恰恰是最不稳的——
   它由单条 finding 驱动,而单条 finding 是否出现对采样最敏感。
   这个改动把区分度从 0.15 提到 0.63,但很可能同时抬高了噪声。
2. **extent / salience 两个新轴**。它们让同类缺陷不再塌陷到同一分,
   但也引入了两个模型可能判不稳的新判断。

两者都必须用信噪比复核,不能因为「分数看起来更合理」就接受。

### 前置条件:先量噪声,再谈区分度

上面五项判据里,`discrimination` 是组间方差比组内方差。
**但只有当"组内"真的被测过时它才有意义。**

第一次真实运行暴露了这个问题:分数由极少数 finding 驱动(每条视频 2–7 条),
一条 major finding 就是 10.00 → 6.50 的跳变。这个粒度下,
同一个模型跑两次可能给出完全不同的分数——那么"模型 A 比模型 B 低 1.5 分"
可能只是重采样,不是差异。

所以**收拢之前必须先跑重复实验**(`scripts/run_variance.py`):
代码、prompt、条件全部固定,关掉缓存,同一批视频跑 N 次,量出

- **组内噪声**:同一模型自己跟自己的分数标准差 = 噪声地板
- **组间信号**:各模型均值之间的标准差
- **信噪比**:后者/前者
- **排序稳定性**:N 次运行给出的模型排序是否一致

两条硬判据:

* **信噪比 < 1 的 aspect 不可能靠加样本救回来。** 它意味着同一个模型跟自己的差异
  比模型之间的差异还大。该改打分粒度或直接 drop,而不是把噪声平均得好看一点。
* **排序在重复中翻转的 aspect 撑不起榜单列**,哪怕单次运行的数字看起来很干净。

噪声地板同时决定了扩样本的规模:要可靠地分辨 Δ 分的差异,
所需样本量大致是 (噪声/Δ)² 的量级。**先测再扩,而不是先扩再祈祷。**
