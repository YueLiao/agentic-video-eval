...
skill 的粒度是为了让它工作(能放大人脸的 skill 不该同时判运镜),但那不是给人看的粒度。
九个数字不是报告,是表格,而且其中一半彼此相关。所以内部分解保持细,**输出固定为六维**:

| 维度 | 含义 | 由哪些 skill 汇入 |
|---|---|---|
| **语义一致性** | 是否做到了条件要求的事:主体、属性、数量、动作、顺序、运镜、风格 | semantic / action / camera / style conformance |
| **运动质量** | 运动本身是否成立:幅度、平滑性、速度合理性、生物力学 | motion_quality |
| **主体保真度** | 主体(尤其是人)的结构:手、脸、四肢、身份一致、材质 | human_integrity |
| **物理合理性** | 世界是否守物理:重力、刚体、穿模、支撑、物体恒存 | physical_integrity |
| **时间一致性** | 画面在时间上是否稳定:闪烁、沸腾、卡顿、突现突消 | temporal_integrity |
| **画面质量** | 单帧本身:结构完整、清晰度、光影逻辑、伪影、美学 | static_integrity |

选这六个的标准是**近似正交**:一段视频可以只挂掉其中任意一个而其余全过。
"整体质量""真实感"这类维度过不了这个检验——没有东西能独立地挂掉它们。

两条聚合规则:

* **多个 skill 汇入同一维度时取最差**,不取平均。维度已经足够粗,
  内部再平均会让一个干净的子项掩盖一个崩掉的子项——那正是拆成六维要防的事。
* **无内容可判的维度报 n/a 并排除在总分外**,不给满分。
  给一段风景片的"主体保真度"打满分,等于奖励视频"不包含难内容";
  一个学到这一点的模型会去生成更空的视频。
