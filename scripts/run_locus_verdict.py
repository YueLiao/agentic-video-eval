...
def main() -> int:
    ...
    def one(job):
        ...
        try:
            ...
            for li, lv in enumerate(res.value["loci"]):
                ...
                if not imgs:
                    continue
                r = vlm.ask(system=SYSTEM, user=PROMPT, images=imgs,
                            schema={"type": "object"},
                            tag=f"lv/{rel}/{lv['locus_id']}")
                p = r.parsed or {}
    ...
    for name, key in (("确认崩坏处数", "n_broken"), ("严重度加权", "severity")):
        sub = [r for r in rows if r["path_A"] in ok and r["path_B"] in ok]
        h = [H[r["MQ"]] for r in sub]
        # Fewer confirmed failures is better, so the model's margin is negated.
        m = [float(ok[r["path_B"]][key] - ok[r["path_A"]][key]) for r in sub]
    ...
    (out / "scores.json").write_text(json.dumps(
        {k: {"n_broken": v.get("n_broken"), "severity": v.get("severity")}
