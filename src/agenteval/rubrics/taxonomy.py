...
_D: list[DefectSpec] = [
    ...
    DefectSpec(
        ...
        {"trace": '手整体正常,只在放大逐帧时发现某一帧里两指边界略有粘连或指尖发糊。**分界**:如果你能明确指出"数量不对"或"某个指节形态错了",就已经是 minor,不是 trace。',
         "minor": '能明确指出问题——某帧手指数量不对,或某指节形态错误——但只出现在个别帧,或手在画面中较小。**分界**:按正常速度播放会不会注意到?会注意到就是 major;只有暂停或放大才发现,就是 minor。',
         "major": '正常速度观看即可看出这只手不对:手指数量在连续多帧中持续错误、指节明显融合成蹼状、或手指朝向违反关节结构。**分界**:手掌整体形状还能不能辨认为一只手?能辨认就是 major;不能就是 severe。',
         "severe": '手已无法被辨认为手:整体成为肉色团块、出现明显非人类的肢状突起、或畸形持续覆盖镜头的大部分时间。'},
    ...
    DefectSpec(
        ...
        "人脸太小看不清时不得下结论。",
        {"trace": '放大后发现轻微不对称,或牙齿/发际线细节略糊,不影响"这是一张正常的脸"的判断。**分界**:能具体说出"哪个器官不对"就是 minor。',
         "minor": '能指出具体器官的问题(如某帧眼睛大小差异明显、牙齿数量异常),但需要留意才察觉。**分界**:第一眼看过去脸是否"不对劲"?是就是 major。',
         "major": '正常观看第一眼即觉得脸不对:五官比例明显失常、眼睛位置不对称到非自然、或牙齿/耳朵结构错误。**分界**:还能不能认出这是一张人脸?能就是 major。',
         "severe": '面部结构崩坏,五官错位或缺失到不再像一张人脸。'},
    ...
    DefectSpec(
        ...
        "遮挡导致的肢体消失也不是。",
        {"trace": '肢体长度或角度有轻微异常,但可以用透视缩短或遮挡合理解释掉大部分。**分界**:排除透视后仍确定异常,就是 minor。',
         "minor": '排除透视因素后仍确认存在长度变化或关节角越界,但幅度小、或只持续一两帧。**分界**:正常速度下会不会看到肢体在"伸缩"或"反折"?会就是 major。',
         "major": '正常观看即可见肢体明显伸缩、关节反向弯折,且持续多帧。**分界**:肢体数量和连接关系是否还正确?正确就是 major。',
         "severe": '多出或缺少肢体,或肢体与躯干的连接位置完全错误。'},
    ...
    DefectSpec(
        ...
        {"trace": '发型走向或衣物褶皱等细节在帧间有变化,但人物明显是同一个。**分界**:面部特征本身发生变化就是 minor。',
         "minor": '面部特征有可察觉的变化(脸型、眼距、肤色),但仍认得出是同一个人。**分界**:如果单看首尾两帧会怀疑"是不是换人了",就是 major。',
         "major": '外观改变到单看首尾会认为是不同的人,或主要衣着中途更换。**分界**:是渐变还是彻底替换?彻底替换成另一张脸是 severe。',
         "severe": '中途完全变成另一个人,前后无法关联。'},
    ...
    DefectSpec(
        ...
        "远景人物细节少是正常的。",
        {"trace": '皮肤纹理略少,但仍能看到毛孔、绒毛等细节层次。**分界**:局部完全没有纹理就是 minor。',
         "minor": '局部区域(如额头、脸颊)完全平滑无毛孔,近看有蜡感,但整体仍像真人皮肤。**分界**:正常观看是否就觉得"这个人不像真人"?是就是 major。',
         "major": '人物皮肤整体呈塑料或蜡质感,正常观看即可察觉不真实。**分界**:是否已经像模型/玩偶而非人?是就是 severe。',
         "severe": '人物如同塑料模型或蜡像,完全失去生物材质的真实感。'},
        "frame", ("highfreq_amplify", "zoom_face")),
    DefectSpec(
        "no_blink", "human_integrity", "无眨眼",
        "有清晰人脸特写且时长足够,但整段中人物完全不眨眼。",
        "片段过短(<3s)、人脸太小、或人物本就闭眼/背对时不适用。",
        {"trace": '有眨眼但频率偏低(真人约每 3-5 秒一次)。**分界**:整段一次都没有就至少是 minor。',
         "minor": '4 秒以上完全无眨眼,但眼睛仍有其他微动(视线移动、眯眼)。**分界**:眼神是否呆滞无变化?是就是 major。',
         "major": '全程无眨眼,且眼神明显呆滞、缺乏聚焦变化。**分界**:整张脸是否都静止?是就是 severe。',
    ...
    DefectSpec(
        ...
        {"trace": '逐帧对比才发现的局部微弱亮度波动,连续播放完全看不出。**分界**:连续播放能看到闪就是 minor。',
         "minor": '连续播放时留意可见轻微明暗跳动,但不干扰观看。**分界**:是否会主动吸引注意力?会就是 major。',
         "major": '正常观看即可见明显闪烁,会吸引注意力并干扰观看。**分界**:是持续全片还是局部片段?持续全片且强烈是 severe。',
         "severe": '全片持续强烈闪烁,严重影响观看甚至引起不适。'},
    ...
    DefectSpec(
        ...
        "关键判据是:**该表面本身应当静止**。",
        {"trace": '小面积表面细节有极轻微蠕动,需放大逐帧才能确认。**分界**:连续播放能看出"在动"就是 minor。',
         "minor": '局部表面(如墙面、衣物)细节持续重排,留意时可见,但面积小。**分界**:是否影响到主体或大片区域?是就是 major。',
         "major": '明显区域持续沸腾,正常观看即可察觉表面"在冒泡",影响画面质感。**分界**:是否已使画面整体失去稳定感?是就是 severe。',
         "severe": '大面积纹理持续崩坏重排,画面整体失去稳定感。'},
    ...
    DefectSpec(
        ...
        {"trace": '单次极短的顿挫,几乎察觉不到,不影响动作理解。**分界**:能明确指出"这里卡了一下"就是 minor。',
         "minor": '有一次可察觉的卡顿或重复帧,但动作整体仍连贯。**分界**:是否出现多次,或造成动作断裂?是就是 major。',
         "major": '多次明显卡顿或时间跳跃,破坏动作连贯性,正常观看即可察觉。**分界**:动作还能不能看懂?能就是 major。',
         "severe": '频繁卡顿使动作无法连贯辨识,视频不可用。'},
    ...
    DefectSpec(
        ...
        {"trace": '背景中极次要的小物件短暂消失,不影响画面理解。**分界**:消失的是可辨识的物体就是 minor。',
         "minor": '次要物体凭空出现或消失,需留意才发现,且与主体动作无关。**分界**:正常观看会不会注意到"刚才那个东西哪去了"?会就是 major。',
         "major": '可辨识的物体凭空出现或消失,正常观看即可察觉,影响场景可信度。**分界**:消失的是不是主体?是就是 severe。',
         "severe": '主体或条件要求的关键物体凭空出现或消失。'},
    ...
    DefectSpec(
        ...
        {"trace": '运动幅度略小于条件描述,但动作完整发生了。**分界**:动作明显没做到位就是 minor。',
         "minor": '运动明显偏弱,动作未完全展开,但仍能看出在做那个动作。**分界**:主体是否基本不动了?是就是 major。',
         "major": '主体基本静止,只有极小幅度移动,与条件描述的动作明显不符。**分界**:是否完全没有执行要求的动作?是就是 severe。',
         "severe": '整段近乎静止,完全没有执行条件要求的动作。'},
    ...
    DefectSpec(
        ...
        "慢动作下动作显得'飘'也是正常的。",
        {"trace": '个别动作略显生硬,但整体运动符合真实规律。**分界**:能指出某个动作违反运动规律就是 minor。',
         "minor": '部分动作不符合真实运动规律(缺乏惯性、重心不转移),留意时可见。**分界**:正常观看是否就觉得"动得很假"?是就是 major。',
         "major": '整体动作明显违反生物力学,正常观看即觉得假:肢体无惯性摆动、重心不随支撑脚转移。**分界**:是否像被逐帧摆出来的?是就是 severe。',
         "severe": '动作完全脱离生物力学,呈提线木偶或逐帧摆拍状。'},
        "sequence", ("ordered_frames", "motion_trail")),
    DefectSpec(
        "speed_anomaly", "motion_quality", "速度异常",
        "运动速度突变、忽快忽慢,或与物体质量/场景明显不符。",
        "条件要求的变速、真实的加减速不算。",
        {"trace": '速度有极轻微波动,不影响动作观感。**分界**:能察觉忽快忽慢就是 minor。',
         "minor": '存在可察觉的速度不均,但动作仍然连贯自然。**分界**:是否出现突变或明显不合理的速度?是就是 major。',
         "major": '明显的速度突变,或速度与物体质量/场景明显不符(如重物瞬间启停)。**分界**:动作还能不能辨识?能就是 major。',
    ...
    DefectSpec(
        ...
        {"trace": '下落轨迹略不自然,但整体方向和加速趋势正确。**分界**:速度或轨迹有明确异常就是 minor。',
         "minor": '下落速度明显偏慢/偏快,或轨迹有可察觉的偏离,但仍在下落。**分界**:是否出现停顿或反向?是就是 major。',
         "major": '物体在空中明显停顿,或轨迹明显违反重力(如水平飘移)。**分界**:是否完全无支撑地悬浮或上升?是就是 severe。',
         "severe": '物体无支撑悬浮不动,或凭空向上运动。'},
    ...
    DefectSpec(
        ...
        "透视变化造成的形状改变也不是形变。",
        {"trace": '刚体边缘有轻微不稳,可能由压缩或运动模糊解释。**分界**:确认形状本身在变就是 minor。',
         "minor": '刚体有可察觉的轻微弯折或伸缩,但整体结构仍然成立。**分界**:正常观看是否就看得出物体在变形?是就是 major。',
         "major": '刚体明显弯折、伸缩或流动,正常观看即可见,违反该材料应有的刚性。**分界**:物体是否还保有原有结构?保有就是 major。',
    ...
    DefectSpec(
        ...
        "半透明物体、影子、倒影不适用。",
        {"trace": '极短暂且极轻微的边缘穿插,可能是遮挡判断误差。**分界**:确认两个实体重叠占据同一位置就是 minor。',
         "minor": '有可察觉的物体互穿,但范围小、时间短。**分界**:正常观看是否会注意到"穿过去了"?会就是 major。',
         "major": '明显的物体互穿并持续可见,破坏空间可信度。**分界**:互穿的是不是主体、幅度是否很大?是就是 severe。',
         "severe": '主体大幅穿过实体,如人物半身穿过桌子。'},
    ...
    DefectSpec(
        ...
        {"trace": '接触点略有偏差,可能由透视或阴影不清导致。**分界**:确认存在可见缝隙或陷入就是 minor。',
         "minor": '脚部或底部与支撑面有可察觉的错位(浮起或陷入),但幅度小。**分界**:正常观看是否看得出"没踩在地上"?是就是 major。',
         "major": '明显悬空或穿入地面,正常观看即可察觉支撑关系不成立。**分界**:是否完全脱离任何支撑?是就是 severe。',
         "severe": '主体完全漂浮于空中,无任何支撑。'},
    ...
    DefectSpec(
        ...
        {"trace": '背景次要物体轮廓略糊,可用景深解释。**分界**:排除虚化后结构本身就是错的,是 minor。',
         "minor": '次要物体结构有可察觉的错误(如椅子腿数量不对),但不在视觉中心。**分界**:是否是可辨识的、观众会看的物体?是就是 major。',
         "major": '可辨识物体的结构明显错误,正常观看即可见几何不成立。**分界**:出问题的是不是主体?是就是 severe。',
         "severe": '主体结构崩坏,不可辨认。'},
    ...
    DefectSpec(
        ...
        {"trace": '放大后才能发现的局部规则纹理。**分界**:正常尺寸下可见网格就是 minor。',
         "minor": '局部可察觉的网格或棋盘花纹,面积小。**分界**:是否吸引注意力、破坏画面?是就是 major。',
         "major": '明显的棋盘/网格伪影,正常观看即可见并破坏画面质感。**分界**:是否覆盖大面积?是就是 severe。',
         "severe": '大面积规则伪影覆盖画面。'},
    ...
    DefectSpec(
        ...
        "低光噪点不是模糊。",
        {"trace": '极小面积的轻微软化,且难以完全排除景深或运动的解释。**分界**:确认该处运动很小却依然模糊,是 minor。',
         "minor": '局部模糊可察觉,且该区域运动很小、景深也解释不了。**分界**:模糊区域是否可辨识、是否影响观看?是就是 major。',
         "major": '可辨识区域明显模糊,无运动或景深可解释,影响画面清晰度。**分界**:模糊的是不是主体、是否持续?是就是 severe。',
         "severe": '主体持续模糊不可辨。'},
    ...
    DefectSpec(
        ...
        {"trace": '阴影方向有极细微偏差,可用多光源解释。**分界**:排除多光源后仍矛盾,就是 minor。',
         "minor": '阴影或反射有可察觉的不一致(如影子方向与主光源不符),需留意才发现。**分界**:正常观看是否觉得光不对?是就是 major。',
         "major": '明显的阴影缺失或方向矛盾,正常观看即觉得光照不成立。**分界**:整体光照逻辑是否还成立?成立就是 major。',
         "severe": '光照逻辑完全崩坏,物体与环境的明暗关系无法自洽。'},
        "frame", ("zoom_grid",)),
    DefectSpec(
        "text_garbled", "appearance_integrity", "文字错乱",
        "画面中的文字字形不成立、拼写错乱,或在帧间变化。",
        "远景、小字、非拉丁/中文的装饰性字符不适用;有意的模糊招牌不算。",
        {"trace": '小字略有变形但仍可识别,不影响阅读。**分界**:字形明显不成立就是 minor。',
         "minor": '文字有可察觉的字形问题(笔画错误、字母变形),但大致可读。**分界**:处于可读位置的文字是否已无法读懂?是就是 major。',
         "major": '可读位置的文字明显错乱,无法读懂内容。**分界**:出错的是不是条件明确要求的文字?是就是 severe。',
         "severe": '条件要求出现的文字完全无法辨认,或内容明显错误。'},
        "frame", ("ocr", "zoom_grid")),
]
